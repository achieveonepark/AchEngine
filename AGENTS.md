...
### 문서 언어 동기화
- 문서를 작성하거나 수정할 때 `Docs~/content/docs`의 모든 언어판(한국어, 영어, 일본어, 중국어)을 함께 작성·갱신한다.
- 언어별 문서에는 동일한 기능, 제약 사항, 설정 방법을 반영한다.
- 문서 생성물인 `Docs~/.next`와 `Docs~/out`은 직접 수정하지 않는다.
