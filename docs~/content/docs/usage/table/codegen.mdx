...
<Callout type="warning" title="스키마 규칙">

- 첫 행의 열 이름은 유효하고 서로 다른 C# 식별자여야 합니다.
- 모든 테이블에는 이름이 정확히 `Id`이고 형식이 `int`인 열이 하나 있어야 합니다.
- 지원하지 않는 형식, 잘못된 셀 값, 중복 ID가 있으면 생성 또는 베이크가 즉시 실패합니다. 오류를 수정한 뒤 다시 실행하세요.
...
- **MemoryPack 없음** → `.json` 파일 (Newtonsoft.Json 직렬화)

베이크된 파일은 `binaryOutputPath`에 저장됩니다.

<Callout type="info" title="Resources 폴더 배치">
