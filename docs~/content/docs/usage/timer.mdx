...
`0`초 타이머와 이미 취소된 토큰은 즉시 완료 또는 취소됩니다. 음수, `NaN`, `Infinity` 지속 시간은 `ArgumentOutOfRangeException`을 발생시킵니다. UniTask가 없어도 `AchTask.Delay`는 `AchTimer`를 통해 게임 시간 기준으로, `DelayRealtime`은 실제 시간 기준으로 동일하게 동작합니다.

## 관련 문서

- [UI 시스템 개요](/ui/)
- [버튼 확장 컴포넌트](/ui/button-extras)
