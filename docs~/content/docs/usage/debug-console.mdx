...
스레드 콜백에서 받은 로그를 큐에 저장한 뒤 안전하게 UI 스레드로 전달합니다.
...
Unity Activity 내부의 네이티브 View로 표시됩니다.

- Unity 화면 위에 네이티브 View로 그려집니다.
- 툴바를 드래그하여 창 위치를 자유롭게 이동할 수 있습니다.
- 화면의 95% 너비, 50% 높이로 상단에 표시됩니다.
- 별도의 시스템 오버레이 권한이 필요하지 않습니다.
...
<Callout type="info" title="Android 권한">

Android 콘솔은 Unity Activity 내부에 붙으므로 `SYSTEM_ALERT_WINDOW` 선언이나 런타임 권한 요청이 필요하지 않습니다.

</Callout>

<Callout type="info" title="성능">

Android와 iOS 모두 네이티브 UI에서 렌더링됩니다. Android 로그의 JNI 전달은 재귀 오류와 프레임 급증을 막기 위해 Unity 메인 스레드에서 프레임당 최대 50개씩 처리됩니다.
