...
`UIRoot`가 런타임 구조를 초기화할 때 `Screen`, `Popup`, `Tooltip` 레이어 루트에 자동으로 추가합니다. `Background`와 `Overlay`는 화면 전체를 덮어야 하므로 Safe Area 적용 대상에서 제외됩니다. 별도 Canvas나 사용자 정의 레이어를 사용할 때만 직접 추가하면 됩니다.

## Unity 6 에디터 툴바

Unity 6000.3 이상에서는 메인 툴바의 **AchEngine › Scene Navigator**에서 Build Settings에 등록된 씬을 바로 선택할 수 있습니다.

- 드롭다운에 Build Settings 씬 목록과 활성화 여부가 표시됩니다.
- **이동** 버튼을 누르면 현재 편집 씬의 저장 여부를 확인한 뒤 선택한 씬을 엽니다.
- 편집 씬을 직접 바꾸거나 Build Settings 목록을 수정하면 선택 상태가 자동으로 갱신됩니다.
- Play Mode 중에는 씬 선택과 이동이 비활성화됩니다.
