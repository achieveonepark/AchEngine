...
`UIRoot`는 `Screen`, `Popup`, `Tooltip` 레이어에 이 컴포넌트를 자동으로 추가합니다. `Background`와 `Overlay`는 전체 화면 레이어이므로 자동 적용되지 않습니다. 별도 Canvas나 사용자 정의 레이어에서만 직접 추가하세요.
...
<Callout type="info" title="커스텀 트랜지션">
