...
> `AchMover`는 Unity의 2D 레이어 충돌 매트릭스를 따르며 Trigger는 이동을 막는 표면에서 제외합니다. 같은 GameObject에 다른 이동 컴포넌트를 함께 두지 마세요.
...
2. **페네트레이션 보정** — 시작 위치의 잔여 겹침을 먼저 밀어냄
3. **Move-and-Slide** — 캡슐 캐스트로 진행 방향 검사 후, 충돌 표면을 따라 슬라이드. 최대 4회 반복으로 코너·다중 표면 처리
4. **계단 자동 등반** — 벽에 막혔을 때 `StepHeight` 이내의 턱이라면 자동으로 올라감 (위로 들어 올림 → 가로 이동 → 다시 아래로)
5. **지면 검사** — 콜라이더 아래로 캡슐 캐스트, 표면 각도가 `MaxSlopeAngle` 이하이면 grounded
6. **지면 스냅** — 경사면을 내려갈 때 표면에서 떨어지지 않도록 다시 밀착
...
> Rigidbody2D가 없으므로 질량은 `1`로 간주합니다. `ForceMode2D.Impulse`는 즉시 속도에 가산되고, `ForceMode2D.Force`는 `Time.fixedDeltaTime`을 곱해 반영됩니다.
...
`AchMover` 자체 충돌은 Unity 물리 콜백이 아니라 캐스트 결과를 사용합니다. 게임플레이 충돌이나 트리거 판정이 필요하면 `Physics2D` 쿼리 또는 별도의 transform 기반 감지 컴포넌트를 사용하세요.
