...
- **`IUIService` / `UIService`** — `Show(id, payload=null)` → `UIView`, `Show<T>(id, payload)` → `T`, `Close(id, closeAll=false)`, `Close(UIView)`, `CloseTopmost()`, `CloseAll()`, `TryGetOpen(id, out view)`/`TryGetOpen<T>(out view)`, `IsOpen(id)`, `Prewarm()`, `Catalog`, `Root`(`UIRoot`), `IsInitialized`. `UIRoot`는 `Screen`·`Popup`·`Tooltip` 레이어에 `UISafeAreaFitter`를 자동으로 붙인다.
