...
감시 폴더의 에셋을 수동으로 다시 스캔하려면 **다시 스캔** 버튼을 클릭하거나,
에디터 메뉴 **AchEngine › Addressables › Rescan Watched Folders** 를 사용합니다.
...
등록한 그룹 이름이 Addressables에 존재하지 않으면 자동으로 그룹을 생성합니다.
그룹 설정(패킹, 빌드 경로 등)은 Addressables Groups 창에서 직접 수정하세요.
