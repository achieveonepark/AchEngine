...
// 타입-세이프 상수 사용 (컴파일 타임 오류 감지)
LocalizationManager.Get(L.Menu.Start);
LocalizationManager.Get(L.Dialog.Confirm);
LocalizationManager.Get(L.Item.Sword.Name);
```

:::tip 자동화
JSON 파일을 변경할 때마다 코드를 재생성해야 합니다.
편집기의 AssetPostprocessor 훅을 이용해 JSON 변경 시 자동 생성하도록 확장할 수 있습니다.
:::
