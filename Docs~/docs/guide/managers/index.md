...
| `AudioManager` | BGM / SFX 재생, 페이드, 뮤트, 3D 공간 음향 |
...
| `PlayerManager` | 플레이어 데이터 컨테이너 관리 (`Add` / `Get` / `Remove`) |
| `SaveManager` | 저장·불러오기·삭제 — `ISaveService` 기반 추상화 (별도 DI 등록 필요) |
| `IAPManager` | Unity IAP 5.3.0 연동 stub (별도 DI 등록 필요) |
